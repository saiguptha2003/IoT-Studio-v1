...
@TriggerBP.route('/getTriggers', methods=['GET'])
@token_required
def getTriggers(userid, email, username):
    try:
        userDoc = getDocument(userid)
        if not userDoc or "triggers" not in userDoc:
            return jsonify({"triggers": []}), 200
        triggersDoc = getDocument(TRIGGERDOC)
        if not triggersDoc:
            return jsonify({"error": "Triggers document not found"}), 404
        triggers_list = []
        for trigger_id in userDoc["triggers"]:
            if trigger_id in triggersDoc:
                trigger_data = triggersDoc[trigger_id]
                triggers_list.append(trigger_data)
        return jsonify({
            "triggers": triggers_list,
            "total_count": len(triggers_list)
        }), 200
    except Exception as e:
        return jsonify({"error": f"An unexpected error occurred: {str(e)}"}), 500
